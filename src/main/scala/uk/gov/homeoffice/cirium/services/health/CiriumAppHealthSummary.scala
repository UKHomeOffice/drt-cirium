...
object CiriumAppHealthSummaryConstructor {
  ...
  def apply(flightStatusActor: ActorRef, portActors: Map[String, ActorRef])(implicit
      executionContext: ExecutionContext
  ): Future[CiriumAppHealthSummary] = {
...
case class AppHealthCheck(
  ...
  def isHealthy(appHealthSummary: CiriumAppHealthSummary): Future[Boolean] = {
    ...
    latestMessageDateTime().map(maybeLatestMessageTime => {
      ...
      val withinHealthThresholds =
        isWithinHealthThresholds(appHealthSummary, maybeLastProcessedMessageDateTime, maybeLatestMessageTime)
