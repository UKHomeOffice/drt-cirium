...
object Cirium {
  ...
  abstract case class Client(appId: String, appKey: String, entryPoint: String, metricsService: MetricsService)(implicit system: ActorSystem) extends CiriumClientLike {
      ...
      makeRequest(uri)
        ...
        .recover {
          case error: Throwable =>
            ...
            metricsService.errorCounterMetric("requestAndUnmarshal-CiriumItemListResponse")
      ...
      Unmarshal[HttpResponse](res)
        ...
        .recover {
          case error: Throwable =>
            ...
            metricsService.errorCounterMetric("requestItem-CiriumFlightStatusResponse")
            CiriumFlightStatusResponseFailure(error)
        }
    })
  }

  class ProdClient(appId: String, appKey: String, entryPoint: String, metricsService: MetricsService)(implicit system: ActorSystem) extends Client(appId, appKey, entryPoint, metricsService) {
  ...
  def amendCiriumFlightStatus(status: CiriumFlightStatus): CiriumFlightStatus = {
    if (
      Set("ABZ", "CWL", "HUY", "INV", "LBA", "SOU", "BOH", "NQY").contains(status.arrivalAirportFsCode.toUpperCase) &&
...
case class BackwardsStrategyImpl(client: CiriumClientLike, targetTime: DateTime, metricsService: MetricsService) extends BackwardsStrategy {
  private val log = LoggerFactory.getLogger(getClass)
    ...
    client.backwards(startItem, 1000).flatMap { c =>
      ...
      firstItem match {
        ...
        case _ =>
          ...
          metricsService.errorCounterMetric("backUntil-dateFromFirstItem")
